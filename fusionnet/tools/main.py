...
def main():
    ...
    if cfg.pytorch.gpu > -1:
        ...
        # swin = swin.cuda(cfg.pytorch.gpu)
    ...
    if cfg.pytorch.test:
        _, preds, av_time = test(0, cfg, test_loader, network, obj_vtx, obj_info, criterions)
    ...
    for epoch in range(cfg.train.begin_epoch, cfg.train.end_epoch + 1):
        ...
        if epoch % cfg.train.test_interval == 0:
            save_model(os.path.join(cfg.pytorch.save_path, 'model_{}.checkpoint'.format(mark)), network,optimizer,epoch)  # optimizer
