...
def train(epoch, cfg, data_loader, model, obj_info, criterions, optimizer=None, swin=None):
    model.train()
    preds = {}
    Loss = AverageMeter()
    ...
    for i, (obj, obj_id, inp, target, loss_msk, trans_local, pose, c_box, s_box, box) in enumerate(data_loader):
        ...
        if cfg.pytorch.gpu > -1:
            ...
            inp_var = inp.cuda(cfg.pytorch.gpu, non_blocking=False).float()#修改后的
            target_var = target.cuda(cfg.pytorch.gpu, non_blocking=False).float()
            loss_msk_var = loss_msk.cuda(cfg.pytorch.gpu, non_blocking=False).float()
            trans_local_var = trans_local.cuda(cfg.pytorch.gpu, non_blocking=False).float()
            pose_var = pose.cuda(cfg.pytorch.gpu, non_blocking=False).float()
            c_box_var = c_box.cuda(cfg.pytorch.gpu, non_blocking=False).float()
            s_box_var = s_box.cuda(cfg.pytorch.gpu, non_blocking=False).float()
        else:
            inp_var = inp.float()
