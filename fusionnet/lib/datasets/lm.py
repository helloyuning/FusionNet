...
class LM(data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            # plt.axis('off')
            # plt.imshow(rgb)
            # plt.savefig('//home/ivclab/path/my_data/all_image/'+str(obj_id)+'_color_'+str(self.count)+'.png')
            # self.count += 1
            #####
            ...
            return obj, obj_id, inp, out, loss_msk, trans_local, pose, c, s, np.asarray(box) #原来的

            # return obj, obj_id, inp, pose, c, s, np.asarray(box), trans_local #收集训练集姿态用
        if self.split == 'test':
            obj = self.load_obj(idx)
            obj_id = ref.obj2idx(obj)
            box = self.load_box(idx)
            ...
            c, s = self.xywh_to_cs(box, self.cfg.augment.pad_ratio, s_max=max(ref.im_w, ref.im_h))
            rgb, c_h_, c_w_, s_ = zoom_in(rgb, c, s, self.cfg.dataiter.inp_res)
            #cv2.imwrite('/home/ivclab/path/EPro-PnP-main/EPro-PnP-6DoF/shu.png', rgb)  # 保存侧视图篇
            rgb = rgb.transpose(2, 0, 1).astype(np.float32) / 255.
            c = np.array([c_w_, c_h_])
            s = s_
            inp = rgb
