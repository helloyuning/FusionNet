...
class FusionNet(BaseBackbone):
    ...
    def __init__(self,
        ...
        self.l2_reshape = nn.Conv2d(160, 128, 1, stride=1, bias=False)
        self.down = nn.Conv2d(d_model, 304, 1, stride=2, bias=False)
        self.reshape = nn.Conv2d(304 * 2, 304, 1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i == 1:
                ...
                x_r = self.reduce(x_l2_reshape)
                bs, c, h, w = x_r.shape
                x_r = x_r.flatten(2).permute(2, 0, 1)
                x_r = self.global_encoder(x_r, pos=self.pos_embedding)
                x_r = x_r.permute(1, 2, 0).contiguous().view(bs, c, h, w)
            if i == 2:
                down = self.down(x_r)
                fusion = torch.cat([x, down], 1)
                x = self.reshape(fusion)
