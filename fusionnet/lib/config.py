...
def get_default_config_pytorch():
    ...
    config.load_model = '/home/ivclab/path/EPro-PnP-main/EPro-PnP-6DoF/epropnp_basic.pth'   # 加载训练好的模型. path to a previously trained model
    config.test = False             # run in test mode or not, 测试的时候开启
    return config

def get_default_dataset_config():
    config = edict()
    config.name = 'lm'# 原始名字为： LineMOD_No_Occ
    config.classes = 'all'
